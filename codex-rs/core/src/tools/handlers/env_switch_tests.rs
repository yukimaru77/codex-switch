...
#[tokio::test]
async fn env_switch_preserves_read_only_remote_policy() {
    ...
    Arc::make_mut(&mut turn.config)
        ...
        .expect("set read-only profile");
    let flags = super::derive_exec_server_policy_flags(&turn);
    assert_eq!(&flags[..2], &["-c", "sandbox_mode=read-only"]);
}
