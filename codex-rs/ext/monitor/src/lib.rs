...
async fn start_monitor(
    ...
    // Match the shell-command interface exposed by monitor_start: the caller
    // intentionally supplies a complete shell command.
    ...
    tokio::spawn({
        ...
        let stopped_for_reaper = Arc::clone(&stopped);
        async move {
            ...
            let cancelled = cancel_reaper.is_cancelled();
            if cancelled {
            ...
            send_exit_event(&state_for_reaper, &name, instance_id, exit_code, cancelled).await;
            stopped_for_reaper.notify_one();
        }
    });
...
async fn stop_monitor(
    ...
    let handle = state
        ...
        .await
        .get(&args.name)
        .map(|handle| (handle.cancel.clone(), Arc::clone(&handle.stopped)));
    let Some((cancel, stopped)) = handle else {
        return Ok(Box::new(JsonToolOutput::new(serde_json::json!({
    ...
    cancel.cancel();
    if tokio::time::timeout(std::time::Duration::from_secs(5), stopped.notified())
        .await
        .is_err()
    {
        return Ok(Box::new(JsonToolOutput::new(serde_json::json!({
            "error": format!("Timed out stopping monitor '{}'.", args.name)
        }))));
    }
